...
public class TopologyInstance {
    ...
    //The long here is the switch DPID,and the tree represents a data structure for the shortest paths from all other
    //switches in the cluster to this particular switch.
    															
    protected Map<Long, Set<NodePortTuple>> clusterBroadcastNodePorts;
    protected Map<Long, BroadcastTree> clusterBroadcastTrees;

    //define the class path cache loader for loading the path to the cache
    protected class PathCacheLoader extends CacheLoader<RouteId, Route> {
        ...
        public Route load(RouteId rid) {
            return ti.buildroute(rid);//The path cache loader obtains the path via topology instance's build-route method
        }
    }
    ...
    protected LoadingCache<RouteId, Route> pathcache;//It is a cache for storing routes according to route id.
    ...
    protected BroadcastTree dijkstra(Cluster c, Long root,
                                     ...
                                     boolean isDstRooted) {
        HashMap<Long, Link> nexthoplinks = new HashMap<Long, Link>();//associate switch and link
        //HashMap<Long, Long> nexthopnodes = new HashMap<Long, Long>();
        HashMap<Long, Integer> cost = new HashMap<Long, Integer>();//associate switch and cost
        int w;
    ...
    protected void calculateShortestPathTreeInClusters() {
        ...
        Map<Link, Integer> linkCost = new HashMap<Link, Integer>();//associate each link with a cost
        int tunnel_weight = switchPorts.size() + 1;//The number of switches in a cluster + 1

        //Assign equal cost to every link in the cluster.
        for(NodePortTuple npt: tunnelPorts) {//for all ports in the cluster
            if (switchPortLinks.get(npt) == null) continue;//if there is no link for such port, skip
            for(Link link: switchPortLinks.get(npt)) {//Otherwise, for every link attached at the port
                if (link == null) continue;
                linkCost.put(link, tunnel_weight);//Add the cost of the link equals to the No.of switches in the cluster
            }
        }

        for(Cluster c: clusters) {
            for (Long node : c.links.keySet()) {//for all switches in the cluster
                BroadcastTree tree = dijkstra(c, node, linkCost, true);//for each switch, create a tree for its shortest
                														//paths to all other switches.
    ...
    protected Route getRoute(ServiceChain sc, long srcId, short srcPort,
        ...
        NodePortTuple npt;
        Route r = getRoute(srcId, dstId, 0);//The path is obtained from this method
        if (r == null && srcId != dstId) return null;//there is no path between src & des

        if (r != null) {
            nptList= new ArrayList<NodePortTuple>(r.getPath());//return the switch port combination along the route
        } else {
            nptList = new ArrayList<NodePortTuple>();
        }
        
        //These four lines are to add the src and dst to the route
    ...
    protected Route getRoute(long srcId, long dstId, long cookie) {
        ...
        try {
            result = pathcache.get(id);//This method obtains the route
        } catch (Exception e) {
            log.error("{}", e);
        }
