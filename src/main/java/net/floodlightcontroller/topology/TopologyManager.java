...
import net.floodlightcontroller.getThroughPut.ICongestionDetectService;
...
public class TopologyManager implements
    ...
    protected ITopologyListener congestionDetect;
   ...
   //  Getter/Setter methods
    ...
    //Here, a Runnable class is defined to provide run() method for a thread to recompute topology.
    /**
    ...
    protected class UpdateTopologyWorker implements Runnable {
        ...
        public void run() {
            try {
                if (ldUpdates.peek() != null)//If there is head in the queue, perform updateTopology()
                    updateTopology();
    ...
    public boolean updateTopology() {
        ...
        List<LDUpdate> appliedUpdates = applyUpdates();//It has a blocking call inside applyUpdates();
        newInstanceFlag = createNewInstance("link-discovery-updates");//create a new topology instance
        lastUpdateTime = new Date();
        informListeners(appliedUpdates);//It informs my CongestionDetect fellow. 
        								//It means that when the CongestionDetect is invoked, a new instance has been created.
    ...
    public void init(FloodlightModuleContext context)
        ...
        congestionDetect = (ITopologyListener) context.getServiceImpl(ICongestionDetectService.class);
        ...
        ldUpdates = new LinkedBlockingQueue<LDUpdate>();//Declare a FIFO queue of LDUpdate
    ...
    public void startUp(FloodlightModuleContext context) {
        ...
        addListener(congestionDetect);
        floodlightProvider.addOFMessageListener(OFType.PACKET_IN, this);//It enables this class to response to Packet_In
    ...
    public List<LDUpdate> applyUpdates() {
        ...
        while (ldUpdates.peek() != null){
        	//retrieve, but does not remove the head of the queue. This statement tests if there is LDUpdate in the queue.
            try {
                update = ldUpdates.take();//If there is, take and retrieve the head of the queue.
            } catch (Exception e) {
            ...
            switch (update.getOperation()) {//Based on the operation of this update, it performs the corresponding updates in the topology
            case LINK_UPDATED:
            ...
            // Add to the list of applied updates. It indicates that this update has been applied.
    ...
    public void informListeners(List<LDUpdate> linkUpdates) {
        ...
        for(int i=0; i<topologyAware.size(); ++i) {
            ...
            if(listener.getClass().getName() == "net.floodlightcontroller.getThroughPut.CongestionDetect")
            {
            	System.out.println("The notification of the change of topology has been sent to CongestionDetect");
            	listener.topologyChanged();
            }
            else
            {
            	System.out.println("There is change in topology, but sth is wrong, the notification cannot be sent to CongestionDetect");
